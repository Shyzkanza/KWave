...
- **Vertical flip / top-anchor** (targeted for a 1.x release, **not yet implemented**) — anchor the
  waves to the **top** of the canvas and fill **upward**, for headers and inverted hero layouts.
...
- **Stateless `KWave(config, phase, time, modifier)` composable** — a pure, deterministic function
  of `(phase, time)` with no internal state, for screenshot tests and perfect external
  synchronization.
...
- **`harmonic` (crest roughness) param on `generate()`** — the natural twin of `crests` (density):
...
- **`spacing`, `variation`, and `seed` parameters on `WaveConfig.generate`** — `spacing` controls
  ...
  pseudo-random jitter that desyncs the layers organically, and `seed` makes that jitter
...
- **Drop-in motion is now in-place breathing, not horizontal drift.** The drop-in `KWave` holds the
  ...
  `phaseShift` remains a live external signal for *deliberate* horizontal translation (e.g. a pager
  offset).
